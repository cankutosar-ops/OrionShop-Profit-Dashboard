...
- The same selected source/window supplies the existing Sales-to-Settlement Difference percentage, charged logistics and storage. Canonical Marketplace Fees are now defined separately from this legacy pricing proxy.
...
Canonical Marketplace Fees are now approved as explicit Finance components, but the forward fee denominator and no-double-count backtest are not. The forward fee resolver therefore remains injectable and its production default remains the Sales-to-Settlement Difference proxy (`Net Sales − Sales API forPay`). Do not switch Smart Pricing to canonical Marketplace Fees until that bounded test is approved. The remaining expected-versus-realized gap, especially on `i8-80444`, cannot be declared resolved by canonical presentation alone.
