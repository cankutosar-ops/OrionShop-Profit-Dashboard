#!/usr/bin/env node
/**
 * Apply Sprint 10 historical_inventory_snapshots migration.
 * Usage: npx tsx scripts/apply-historical-inventory-snapshots-migration.mjs
 */
...
const sql = readFileSync(
  resolve(process.cwd(), "supabase/migrations/20260726140000_historical_inventory_snapshots.sql"),
  "utf8"
);
...
if (!result.applied) {
  console.error("FAILED to apply migration. Apply SQL manually in Supabase SQL Editor.");
...
const { error } = await sb.from("historical_inventory_snapshots").select("id").limit(1);
if (error) {
  console.error("Probe failed:", error.message);
  process.exit(2);
}
console.log("Probe OK — historical_inventory_snapshots readable");
