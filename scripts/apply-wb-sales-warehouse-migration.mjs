#!/usr/bin/env node
/**
 * Apply wb_sales.warehouse migration to remote Supabase.
 * Usage: npx tsx scripts/apply-wb-sales-warehouse-migration.mjs
 */
...
async function hasWarehouseColumn() {
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const key = process.env.SUPABASE_SERVICE_ROLE_KEY;
  const client = createClient(url, key, { auth: { persistSession: false } });
  const { error } = await client.from("wb_sales").select("warehouse").limit(1);
  return !error;
}
...
async function main() {
  ...
  const migrationPath = resolve(
    process.cwd(),
    "supabase/migrations/20260720170000_wb_sales_warehouse.sql"
  );
  const sql = readFileSync(migrationPath, "utf8");

  console.log("=== wb_sales.warehouse migration ===\n");

  const before = await hasWarehouseColumn();
  console.log(`warehouse column before: ${before ? "PRESENT" : "MISSING"}`);
  ...
  if (!before) {
    ...
    console.log("Column already present — migration skipped.");
  }

  const after = await hasWarehouseColumn();
  console.log(`warehouse column after: ${after ? "PRESENT" : "MISSING"}`);

  if (!after) {
    console.error("\nFAIL: wb_sales.warehouse still missing after apply.");
    process.exit(1);
  }

  console.log("\nOK: wb_sales.warehouse is available.");
}
