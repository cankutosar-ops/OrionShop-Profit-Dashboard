  ...
  try {
    ...
    if (reports.length) {
      const rows = reports.map((report) => ({
        ...
        delivery_service_sum: parseMoney(report.deliveryServiceSum),
        paid_storage_sum: parseMoney(report.paidStorageSum),
        paid_acceptance_sum: parseMoney(report.paidAcceptanceSum),
        deduction_sum: parseMoney(report.deductionSum),
        penalty_sum: parseMoney(report.penaltySum),
        additional_payment_sum: parseMoney(report.additionalPaymentSum),
        cashback_amount_sum: parseMoney(report.cashbackAmountSum),
        cashback_discount_sum: parseMoney(report.cashbackDiscountSum),
        cashback_commission_change_sum: parseMoney(report.cashbackCommissionChangeSum),
        payment_schedule: parseMoney(report.paymentSchedule),
