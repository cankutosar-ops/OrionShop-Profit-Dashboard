...
export async function listWarehouseSalesReportSnapshots(
  ...
  return (data ?? []).map((row) => ({
    ...
    deliveryServiceSum: num(row.delivery_service_sum),
    paidStorageSum: num(row.paid_storage_sum),
    paidAcceptanceSum: num(row.paid_acceptance_sum),
    deductionSum: num(row.deduction_sum),
    penaltySum: num(row.penalty_sum),
    additionalPaymentSum: num(row.additional_payment_sum),
    cashbackAmountSum: num(row.cashback_amount_sum),
    cashbackDiscountSum: num(row.cashback_discount_sum),
    cashbackCommissionChangeSum: num(row.cashback_commission_change_sum),
    paymentSchedule: num(row.payment_schedule),
