/**
 * Orion Knowledge sources — read-only (Sprint 12.8).
 */

import { NextResponse } from "next/server";
import { requireAuth, isAuthFailure } from "@/lib/security/require-auth";
import { getKnowledgeSources, isKnowledgeAccessible } from "@/services/orion-knowledge-service";
...
export async function GET(request: Request, { params }: Params) {
  ...
  const sources = getKnowledgeSources(id);
  if (!sources) {
